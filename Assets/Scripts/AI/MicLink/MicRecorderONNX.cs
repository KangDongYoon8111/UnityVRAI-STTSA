...
public class MicRecorderONNX : MonoBehaviour
{
    [Header("설정")]
    public bool isSaveToWav = true; // 이 값에 따라 WAV 저장 or float[] 추출
    ...
    public float[] pcmData { get; private set; } // Whisper에 넘겨줄 PCM 데이터
    ...
    public void StartRecording()
    {
        ...
        // 안전한 방식
        Invoke(nameof(StopRecording), maxRecordTime); // 컴파일 시점에 StopRecording이 존재하는지 검사함
        // 구식 방식
        //Invoke("StopRecording", maxRecordTime); // 오타 있어도 컴파일 통과 -> 런타임 오류 발생 가능
    }
    ...
    public void StopRecording()
    {
        ...
        {
            ExtractPcmFromClip(recordedClip); // 녹음된 오디오를 float[] PCM 형식으로 추출
        }        
    }
    ...
    private void ExtractPcmFromClip(AudioClip clip)
    {
        // 오디오 데이터의 전체 샘플 수를 계산
        // 예: 5초 녹음, 샘플레이트 16000, 채널 수 1일 경우 -> 16000 * 5 * 1 = 80000
        int totalSamples = clip.samples * clip.channels;

        pcmData = new float[totalSamples]; // PCM 데이터를 담을 float 배열을 준비
        clip.GetData(pcmData, 0); // AudioClip에서 실제 PCM(float) 데이터를 읽어오기, 0은 시작위치(처음부터 추출)

        Debug.Log($"PCM 추출 완료 ({totalSamples}개 샘플)");
        debugText.text = $"PCM 추출 완료 ({totalSamples}개 샘플)";
    }
